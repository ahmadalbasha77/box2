...
class OfferProductController extends GetxController {
  static OfferProductController get to => Get.isRegistered<OfferProductController>()
      ? Get.find<OfferProductController>()
  ...
  void onInit() {
    ...
    pagingController.addPageRequestListener((pageKey) {
      getOfferProduct(pageKey: pageKey);
    });

    controllerSearch.addListener(searchOnChange);
  }

  void getOfferProduct({required int pageKey}) async {
    try {
      currentPageSize++;
      final result = await RestApi.getOfferProduct(
        pageSize: 10,
